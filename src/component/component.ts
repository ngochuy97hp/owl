...
export const enum STATUS {
  ...
  STARTED,          // willStart is completed
  READY,            // has been patched at least once
  MOUNTED,          // is ready, and in DOM
  DESTROYED,
}
...
export class Component<Props extends {} = any, T extends Env = Env> {
  ...
  async __prepareAndRender(fiber: Fiber, cb: CallableFunction) {
    try {
      ...
      this.__owl__.status = STATUS.STARTED;
    } catch (e) {
      fiber.handleError(e);
      return Promise.resolve();
    }
